...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public String joinRoom(Long user_id, Long room_id) throws Exception {
        try{
            ...
            if(chatEntity.getIsPrivate() == 0){
                Set<ChatEntity> joinedRoomSet = userEntity.getJoinedRooms();
                joinedRoomSet.add(chatEntity);
                userEntity.setJoinedRooms(joinedRoomSet);
                chatRepository.insertUserIntoRoom(room_id, user_id);
                return "User " + userEntity.getUserName() + " has joined room " + chatEntity.getRoomName();
            }
            else{
                return "User " + userEntity.getUserName() + " cannot join to this room";
            }
        }
        catch(Exception e) {
            throw new Exception(e.getMessage());
