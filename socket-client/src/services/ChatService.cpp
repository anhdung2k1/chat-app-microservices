    ...
    while(!exitApp) {
        if (!ResponseReceive(sock, buffer)) {
            ...
            if (!exitApp) {
                ...
                // ResponseReceive(sock, buffer);
                // HandlePattern(buffer, sock);
                // GetValueFromServer(sock, "RECONNECT");
                // nlohmann::json resendMsg = nlohmann::json::array();
                // while(!pendingMsg.empty()) {
                //     resendMsg.insert(resendMsg.end(), pendingMsg.front());
                //     pendingMsg.pop();
...
// bool ChatService::selectRoom(int sock, ChatClient& room, int exitValue,string roomSelect) {
//     checkingUpdate = true;
//     thread checkUpdate(GetValueFromServer, sock, "RELOAD_ROOM");
//     // string n;
//     // cout << "\n\n\033[1;32mSelect room id: \033[0m";
//     // cin >> n;
//     // cin.ignore();
//     // HandleInput();
//     checkingUpdate = false;
//     checkUpdate.join();
//     return joinRoom(sock, roomSelect, room);
// }
...
// map<int, ChatClient> ChatService::GetListPrivateRoom(int sock, vector<ChatClient>& listChatRooms, bool& isSuccess){
//     listChatRooms.clear();
...
// bool ChatService::Reconnect(int sock) {
//     int con;
...
// void ChatService::checkConnect(int sock, char* buffer) {
//     bzero(buffer, 1024);
//     int valread = recv(sock, buffer, 1024, 0);
//     if (valread <= 0) {
//         cout << "\nServer crashed!" << endl;
...
//test
// void ChatService::waitConnect(){
//     cout << "waiting for reconnect.. " << endl;
...
bool ChatService::HandleName(string username){
    ...
    for(int i = 0; i <= username.length(); i++){
        if(isspace(username[i]) ) {
            if(isspace(username[i+1])){
               //cout <<" Do not enter more than 2 space characters "<<endl;
                return false;
            }
        }
    }
    for(int i = 0; i < username.length(); i++){
        ...
        //cout << " Special characters are not allowed : " << username[i] <<endl;
        return false;
    }
    return true;
}
